...
import { verifyArcjetConnection } from "./configs/arcjet.config.ts";
...
process.on("SIGINT", async () => {
	await prisma.$disconnect();
	Logger.log("\nSIGINT received: Prisma Client disconnected. Shutting down.", chalk.redBright.italic);
    process.exit(0);
});
...
process.on("SIGTERM", async () => {
	await prisma.$disconnect();
	Logger.log("\nSIGTERM received: Prisma Client disconnected. Shutting down.", chalk.redBright.italic);
	process.exit(0);
});
