    ...
    try {
        ...
        const newClassroom = await prisma.$transaction(async (txn) => {
            const classroom = await txn.classroom.create({
                data: { name, batch, subject, joiningCode },
            });
...
export const joinClassroom = async (
    ...
    try {
        ...
        if (!joiningCode.trim()) throw new BadRequestError("Joining code is required.");

        const classroomToJoin = await prisma.classroom.findUnique({
            where: { joiningCode: joiningCode },
        });
        if (!classroomToJoin) throw new NotFoundError("Classroom not found.");

        const existingMember = await prisma.classroomMember.findFirst({
            where: { userId: studentId, classroomId: classroomToJoin.id },
        });
        if (existingMember) throw new ConflictError("Existing member to the classroom.");
...
export const getClassroomById = async (
    ...
    try {
        ...
        const classroom = await prisma.classroom.findUnique({
            where: { id: classroomId.trim() },
            include: {
                members: {
                    include: {
                        user: { select: { id: true, fullName: true, accountType: true, profilePhotoUrl: true } },
                    },
                },
            },
        });
        ...
        if (userRole === "STUDENT") {
            classroom.members = classroom.members.filter(
                (member) => ["CREATOR", "CO_TUTOR"].includes(member.role) || member.userId === userId,
            );
        }
...
export const updateClassroom = async (
    ...
    try {
        const { classroomId } = req.params as { classroomId: string };
        if (!classroomId.trim()) throw new BadRequestError("Classroom ID is required.");
        
        const { name, subject, batch } = req.body as Partial<{ name: string; subject: string; batch: string; }>;
        if (![name, subject, batch].some((field) => field?.trim())) throw new BadRequestError("Nothing to update.");
...
export const leaveClassroom = async (
    ...
    try {
        ...
        const userId = req.user?.id as string;
        if (!classroomId.trim()) throw new BadRequestError("Classroom ID is required.");
        if (req.membership?.role === "CREATOR") throw new ForbiddenError("The classroom creator cannot leave.");

        await prisma.classroomMember.delete({
            where: { userId_classroomId: { userId, classroomId } },
        });
        res.status(200).json({ success: true, message: "You have successfully left the classroom" });
    } catch (error) {
        next(error);
    }
};
 ...
 * @returns {Promise<void>}
 */
export const inviteCoTutor = async (
    req: AuthenticatedRequest,
    res: Response,
    next: NextFunction,
): Promise<void> => {
    try {
        ...
        // 🚨 SECURITY CHECK: Ensure they are still globally verified
        const latestApplication = await prisma.tutorApplication.findFirst({
        ...
        res.status(201).json({ success: true, message: `Invitation sent to ${inviteeEmail}` });
    } catch (error) {
        next(error);
    }
};
 ...
 * @returns {Promise<void>}
 */
export const refreshJoiningCode = async (
    req: AuthenticatedRequest,
    res: Response,
    next: NextFunction,
): Promise<void> => {
    try {
        const { classroomId } = req.params as { classroomId: string };
        if (!classroomId?.trim()) throw new BadRequestError("Classroom ID id required.");
...
export const transferOwnership = async (
    ...
    try {
        ...
        const newOwnership = await prisma.classroomMember.findUnique({
            where: { userId_classroomId: { userId: newOwnerId, classroomId } },
        });
        ...
        res.status(200).json({ success: true, message: "Classroom ownership transfered successfully." });
    } catch (error) {
        next(error);
    }
};