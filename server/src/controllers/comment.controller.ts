 ...
 */
export const getCommentsForAnnouncement = async (
    ...
    try {
        ...
        if (![announcementId, authorId, text].every((field) => field.trim())) {
            throw new BadRequestError(
                "Announcement ID, author ID, text are required.",
            );
        }
        ...
        /** @section Real-time Update (Async) */
        try {
            // Fetch the announcement to get the classroomId
            ...
            if (announcement) {
                notifyNewComment(announcement.classroomId, newComment);
            }
        } catch (notifyError) {
            console.error("Failed to broadcast new comment:", notifyError);
        }
    } catch (error: any) {
        /**
...
export const updateComment = async (
    ...
    try {
        ...
        if (![commentId, text, userId].every((field) => field.trim())) {
            throw new BadRequestError(
                "Comment ID, user ID, text are required.",
            );
        }
...
export const deleteComment = async (
    ...
    try {
        ...
        if (
            ![commentId, userId, membershipRole].every((field) => field.trim())
        ) {
            throw new BadRequestError(
                "Comment ID, user ID, membership role are required.",
            );
        }
