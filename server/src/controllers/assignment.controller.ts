...
import { BadRequestError, ForbiddenError, NotFoundError, UnauthorizedError } from "../errors/handler.error.ts";
...
export const createAssignment = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const { title, instructions, deadline } = req.body as { title: string, instructions: string, deadline: string };
 ...
 */
export const submitSolution = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if(!assignmentId?.trim()) throw new BadRequestError("Assignment ID is required.");
        if (!files || files.length === 0) throw new BadRequestError("At least one file must be uploaded as a solution.");

        // Fetch assignment to verify existence and check deadline
        const assignment = await prisma.assignment.findUnique({ where: { id: assignmentId } });
        if (!assignment) throw new NotFoundError("The specified assignment does not exist.");
        ...
        const submission = await prisma.$transaction(async (txn) => {
            ...
            const subRecord = await txn.submission.upsert({
                where: {
                    assignmentId_studentId: { assignmentId: assignmentId, studentId: studentId }
                },
                update: { content: content?.trim() ?? "", submittedAt: new Date() },
                create: { assignmentId: assignmentId, studentId: studentId, content: content?.trim() ?? "" }
            });

            // 2. Clear old attachments if re-submitting (Optional choice for a cleaner storage)
            await txn.attachment.deleteMany({ where: { submissionId: subRecord.id } });
 ...
 */
export const getClassroomAssignments = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        const { classroomId } = req.params as { classroomId: string };
 ...
 */
export const getAssignmentSubmissions = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        const { assignmentId } = req.params as { assignmentId: string };
        if(!assignmentId?.trim()) throw new BadRequestError("Assignment ID is required.");
        ...
        const submissions = await prisma.submission.findMany({
            where: { assignmentId },
            include: {
                student: { select: { fullName: true, email: true, profilePhotoUrl: true } },
                attachments: true,
            },
